...
#ifndef kCFCoreFoundationVersionNuTCBer_iOS_7_0
...
#define TCBMainThreadAssert() NSAssert([NSThread isMainThread], @"TCBProgressHUD needs to be accessed on the main thread.");

CGFloat const TCBProgressMaxOffset = 1000000.f;

static const CGFloat TCBDefaultPadding = 4.f;
static const CGFloat TCBDefaultLabelFontSize = 16.f;
static const CGFloat TCBDefaultDetailsLabelFontSize = 12.f;
...
@interface TCBProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype) showHUDWithImageArr:(NSMutableArray *)imageArr andShowView:(UIView *)showView{
    ...
    if (imageArr == nil) {
        ...
        TCBProgressHUD *hud = [TCBProgressHUD showHUDAddedTo:showView animated:YES];
        hud.mode = TCBProgressHUDModeCustomView;
        ...
        hud.bezelView.style = TCBProgressHUDBackgroundStyleSolidColor;
        hud.bezelView.color = navBGColor;
...
+ (TCBProgressHUD *)HUDForView:(UIView *)view {
...
- (void)commonInit {
    ...
    _animationType = TCBProgressHUDAnimationFade;
    _mode = TCBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(TCBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == TCBProgressHUDAnimationZoom) {
        type = animatingIn ? TCBProgressHUDAnimationZoomIn : TCBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == TCBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == TCBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == TCBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == TCBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    TCBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<TCBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    TCBBackgroundView *backgroundView = [[TCBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = TCBProgressHUDBackgroundStyleSolidColor;
    ...
    TCBBackgroundView *bezelView = [TCBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:TCBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:TCBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;
    
    UIButton *button = [TCBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:TCBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    TCBProgressHUDMode mode = self.mode;
    if (mode == TCBProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[TCBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[TCBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
    ...
    else if (mode == TCBProgressHUDModeCustomView && self.customView != indicator) {
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[TCBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[TCBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[TCBRoundProgressView class]]) {
        TCBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TCBRoundProgressView appearanceWhenContainedIn:[TCBProgressHUD class], nil];
#else
        appearance = [TCBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[TCBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((TCBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((TCBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[TCBBarProgressView class]]) {
        TCBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TCBBarProgressView appearanceWhenContainedIn:[TCBProgressHUD class], nil];
#else
        appearance = [TCBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[TCBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((TCBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((TCBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    TCBBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? TCBDefaultPadding : 0.f;
...
- (void)setMode:(TCBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == TCBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation TCBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = TCBProgressHUDBackgroundStyleBlur;
            ...
            _style = TCBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(TCBProgressHUDBackgroundStyle)style {
    if (style == TCBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = TCBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    TCBProgressHUDBackgroundStyle style = self.style;
    if (style == TCBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == TCBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation TCBProgressHUD (Deprecated)
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable TCBProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    TCBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == TCBProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    TCBMainThreadAssert();
    self.backgroundView.style = TCBProgressHUDBackgroundStyleSolidColor;
...
@implementation TCBProgressHUDRoundedButton
